...
function ListPosts() {
  ...
  const partofroute = '/selectedpost'
  const partofrouteedit = '/editpost'
    ...
    if (confirm('Are you sure?')) {
      ...
      fetch('https://jsonplaceholder.typicode.com/posts/' + id, {
        ...
        .then(async (response) => {
          ...
          if (myElement !== null) myElement.outerHTML = ''
        })
        .catch((error) => {
            ...
            {totalPosts &&
              totalPosts.map((post) => (
                <tr key={post.id} id={post.id}>
                  ...
                  <td>
                    {' '}
                    <Link to={`${partofrouteedit}/${post.id}`}>Edit</Link>
                  </td>
                  <td>
                    {' '}
                    <Link to={`${partofroute}/${post.id}`}>{post.title}</Link>
                  </td>
                </tr>
              ))}
          </tbody>
